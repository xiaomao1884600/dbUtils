...
public class CityDictionaryHelper {

    public static JSONArray getTongchengProvinces() {
        JSONArray independentCities = processIndependentCities();
        independentCities.addAll(processProvinces());
        return independentCities;
    }

    private static JSONArray processIndependentCities() {
        JSONArray independentCities = new JSONArray();
        String[] independentCitiesArray = INDEPENDENT_CITIES.split(",");
        for (String cityString : independentCitiesArray) {
            JSONObject provinceLevelObject = new JSONObject();

            JSONObject cityObject = new JSONObject();
            String name = StringUtils.substringBefore(cityString,":");
            String code = StringUtils.substringBetween(cityString, ":", "|");
            String id = StringUtils.substringAfter(cityString, "|");
            cityObject.put("id", id);
            cityObject.put("name", name);
            cityObject.put("code", code);


            JSONArray citySolely = new JSONArray();
            citySolely.add(cityObject);
            provinceLevelObject.put(name, citySolely);

            independentCities.add(provinceLevelObject);
        }
        return independentCities;
    }

    private static JSONArray processProvinces() {
        JSONArray provinces = new JSONArray();
        JSONObject provinceObjects = JSONObject.fromObject(PROVINCES);

        Set provinceNames = provinceObjects.keySet();

        for (Object provinceName : provinceNames) {
            JSONObject cityUnderProvinceUnprocessed = provinceObjects.getJSONObject(provinceName.toString());
            JSONObject cityUnderProvinceProcessed = processCity(cityUnderProvinceUnprocessed, provinceName.toString());
            provinces.add(cityUnderProvinceProcessed);
        }

        return provinces;
    }

    private static JSONObject processCity(JSONObject cityUnderProvinceUnprocessed, String provinceName) {
        JSONObject province = new JSONObject();

        JSONArray cities = new JSONArray();
        Set cityNames = cityUnderProvinceUnprocessed.keySet();
        for (Object cityName : cityNames) {
            JSONObject city = new JSONObject();
            String cityContent = cityUnderProvinceUnprocessed.getString(cityName.toString());
            String code = StringUtils.substringBefore(cityContent, "|");
            String id = StringUtils.substringAfter(cityContent, "|");
            city.put("id", id);
        ...
        province.put(provinceName, cities);

        return province;
    }

    private static final String INDEPENDENT_CITIES = "北京:bj|1," +
            "上海:sh|2," +
            "天津:tj|18," +
            "重庆:cq|37";
    ...
    public static void main(String[] args) {
        System.out.println(getTongchengProvinces().toString());
    }
}
